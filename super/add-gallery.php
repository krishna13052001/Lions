...
if (!isset($_SESSION['super-id'])) {
    ...
    $sliders = $con->query("SELECT * FROM `images` WHERE `category` = 'gallery'");
    ...
    if (isset($_POST['add-gallery'])) {
        foreach ($_FILES["image"]["tmp_name"] as $key => $tmp_name) {
            ...
            if (in_array(strtolower($ext), $allowTypes)) {
                ...
                $con->query("INSERT INTO `images`(`img`, `category`, `categoryId`) VALUES ('$compressedImage', 'gallery', '0')");
                header("Location: ./add-gallery.php");
            }
        }
    } else if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $img = $con->query("SELECT * FROM `images` WHERE `id` = '$id'");
        $img = $img->fetch_assoc();
        unlink($img['image']);

        if ($con->query("DELETE FROM `images` WHERE `id` = '$id'")) {
            header("Location: ./add-gallery.php");
        }
    }
}
...
<body>
    ...
    <section class="members">
      ...
      <div class="row">
        ...
        <div class="col-lg-4">
          <form action="./add-gallery.php" method="post" class="form-group container" enctype="multipart/form-data">
            <div style="text-align:center;">
              <h2 class="animated">Add Gallery Image</h2>
            </div>
          ...
          <div class="col-lg-4" >
            <img src="<?= $img['img'] ?>" alt=""
                ...
                <div style="text-align:center;padding: 20px;">
                  <button href="./add-gallery.php?id=<?= $img['id'] ?>" class="btn btn-outline-danger" type="button" name="button">Delete</button>
                </div>
